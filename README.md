...
3. Open [https://connectx.online](https://connectx.online) in your browser. (When running locally: [http://localhost:3000](http://localhost:3000))
...
Users can log in with their Gmail account. Create OAuth credentials in [Google Cloud Console](https://console.cloud.google.com/): add redirect URIs `http://localhost:3000/auth/google/callback` (local) and `https://connectx.online/auth/google/callback` (production), then set `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and `SESSION_SECRET` in `.env`. Set `BASE_URL` to `https://connectx.online` for production.
